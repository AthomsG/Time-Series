...
# Fit plot function
fit_plot <- function(fit_obj, x, title = "") {
  ...
  fit_df$Fitted <- fitted(fit_obj)
  
  p <- ggplot(fit_df, aes(x = Time)) +
    geom_line(aes(y = Value, color = Type), linewidth = 0.5, alpha=0.8) +
    geom_line(aes(y = Fitted, color = "Fitted"), linewidth = 0.8, linetype = "dashed", alpha=0.7) +
...
forecast_plot <- function(forecast_obj, x, title = "", n_points = 10) {
  ...
  if (!is.null(title)) {
    ...
    ggsave(filename, p, device = "pdf", width = 6, height = 4)
  }
...
# Plot the fit
fit_plot(Olivais_fit, Olivais, title = "")
...
# Plot the fit
fit_plot(alfragide_fit, alfragide, title = "")
...
fit_plot(beato_fit, beato, title = "")
...
fit_plot(entrecampos_fit, entrecampos, title = "")
...
fit_plot(Reboleira_fit, Reboleira, title = "")
